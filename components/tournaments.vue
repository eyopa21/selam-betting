...
async function getTournamentForSport(countryId: number) {
...
async function getCountries() {
  if (isLoading.value || !hasMore.value) {
    return
  } // Prevent multiple concurrent fetches or fetching when no more data
...
<template>
  <div class="">
    ...
    <div v-else>
      ...
      <q-infinite-scroll v-else :offset="500" class="tw-h-32  tw-max-h-64" @load="loadMore">
        <div>
          <NavPopularCountries />
        </div>
        <q-tree
          default-expand-all :nodes="lazy" dense no-connectors text-color="white" color="white" node-key="label"
          class="tw-mx-4" @lazy-load="onLazyLoad"
        >
          <template #default-header="prop">
            <div class="row items-center  tw-p-1">
              <div class="text-weight-bold tw-cursor-pointer ">
                {{ prop.node.label }}
              </div>
            </div>
          </template>
